defmodule Converter do
  ...
  def to_light_seconds({:miles, miles} = _, precision: precision) when is_integer(miles) or is_float(miles) do
    (miles * 5.36819e-6) |> round_to(precision)
  end

  def to_light_seconds({:meters, meters} = _, precision: precision) when is_integer(meters) or is_float(meters)  do
    (meters * 3.335638620368e-9) |> round_to(precision)
  end

  def to_light_seconds({:feet, feet} = _, precision: precision) when is_integer(feet) or is_float(feet)  do
    (feet * 1.016702651488166404e-9) |> round_to(precision)
  end

  def to_light_seconds({:inches, inches} = _, precision: precision) when is_integer(inches) or is_float(inches)  do
